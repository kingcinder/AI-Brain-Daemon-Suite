...
if [ -n "$EMOTION" ]; then
  ...
  case $EMOTION in
    joy|happiness|delight|excitement|satisfaction)
    ...
    patience|tolerance)
      ...
      ;;
    *)
      echo "⚠️  '$EMOTION' isn't a recognized emotion label — the entry was logged, but no dimension was adjusted."
      echo "   Recognized labels: joy, happiness, delight, excitement, satisfaction, sadness, disappointment,"
      echo "   melancholy, anger, frustration, irritation, fear, anxiety, worry, calm, peace, contentment,"
      echo "   curiosity, interest, fascination, connection, warmth, affection, loneliness, disconnection,"
      echo "   fatigue, tiredness, exhaustion, energized, alert, refreshed, anticipation, eagerness,"
      echo "   looking-forward, disappointment-expectation, letdown, trust, safety, secure, betrayal, hurt,"
      echo "   violated, impatience, annoyance, patience, tolerance"
      ;;
  esac
