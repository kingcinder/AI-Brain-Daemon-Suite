...
# Self-initialize with install.sh's exact schema if the state file doesn't
# exist yet — unlike resolve-check.sh/get-lessons.sh (where "nothing to do
# yet" is a fine response to missing state), this script's whole job is to
# record a NEW error. Silently doing nothing here would drop a real report;
# erroring with a raw Python traceback (the prior bug) isn't much better.
# Held under the same flock acquired above, so a concurrent run can't race
# on the create.
if [ ! -f "$STATE_FILE" ]; then
    echo "No acc-state.json found — initializing it now"
    cat > "$STATE_FILE" << 'EOF'
{
  "version": "2.0",
  "lastUpdated": null,
...
# Sync state to markdown (this module's own sync-state.sh, not a sibling's)
