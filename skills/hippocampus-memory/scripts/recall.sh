...
export RECALL_INDEX="$INDEX"
export RECALL_QUERY="$QUERY"
export RECALL_TOP="$TOP"
export RECALL_MIN_SCORE="$MIN_SCORE"
...
INDEX_PATH = os.environ["RECALL_INDEX"]
QUERY = os.environ["RECALL_QUERY"].lower()
TOP = int(os.environ["RECALL_TOP"])
MIN_SCORE = float(os.environ["RECALL_MIN_SCORE"])
...
if not query_terms:
    print(f"⚠️  Query '{QUERY}' has no terms longer than 2 characters — nothing to search on.")
    raise SystemExit(0)
