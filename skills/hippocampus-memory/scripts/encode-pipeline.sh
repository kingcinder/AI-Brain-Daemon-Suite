...
if [ "$PENDING_COUNT" -eq 0 ]; then
    ...
    "$SKILL_DIR/scripts/log-event.sh" encoding signals_pending=0 2>/dev/null || true
    exit 0
fi
...
"$SCRIPT_DIR/log-event.sh" encoding signals_pending="$PENDING_COUNT" 2>/dev/null || true
