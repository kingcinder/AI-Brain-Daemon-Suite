...
| File | Verdict | Notes |
|---|---|---|
| `deep-brain-kernel.py` | 🔧 | Spawn jobs route through `core/spawn/spawn-provider.sh` → the suite's own local `llm-call.sh` / internal agentic loop (**`SPAWN_PROVIDER=agentloop` is the default since 2026-08-22** — Initiative 4) or `hermes chat -q … --source daemon` when `SPAWN_PROVIDER=hermes` is explicitly set (see the Gap-2 follow-on below). Fixed 3 legacy `sessions:spawn` comment/output strings → `hermes chat` (Job dataclass comment, `--check` column label, `_await_with_timeout` docstring). `--check` still validates `hermes` on PATH (informational) + the provider shim. |
